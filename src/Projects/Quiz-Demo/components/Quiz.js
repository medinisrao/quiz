...
// console.log(quetionStatus)
  ...
  const handleMatchClick = (e,index)=>{
    ...
    return <>
        { showReport ? <Report score={score} quetionsLength={quetionsLength} userSelectedQuetions={userSelectedQuetions}></Report> : <div className='quiz-wrapper'>
           
           <div className='btn-container'>
            {
                    quetionsForBtn.map((item,index)=>{
                    
                        return <button className={setClassName(index) } onClick={handleIndexBtn}>{index+1}</button>
                    })
                }
           </div>
           ...
           {quetions.options.map((option,index)=>{          
                    return  <button className={setClassForClicked(index)} style={{pointerEvents:setPointerEvents(index)}}  data-index={index} onClick={(e)=>handleClick(e,index)} disabled={quetionStatus[quetionIndex].disabled}>
                    {option}
                </button>
               })}
                        ...
                        <div className='left'>
                                {
                                    quetions.leftQuetions.map((item,index)=>{
                                        return <span className='left-wrapper'>
